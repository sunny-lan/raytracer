...
Vector3 origin = new(-5, 3, -5), lookAt = new Vector3(0, 0, 0);
Camera camera = new(
    fov: 30,
    aspect: aspRatio,
    lookAt,
...
Vector3 RayColor(in Ray r, in int depth, int samples)
{
    if (depth <= 0) return Vector3.Zero;

    if (world.Hit(r, 0.001, double.PositiveInfinity, out var hit))
    {
        Vector3 color = hit.Material.Emit(r, hit);

        Vector3 reflectionEstimate = Vector3.Zero;
        for(int i = 0; i < samples; i++)
        {
            Vector3 direction = Util.RandomInUnitSphere().Normalized();
            direction.Y = Math.Abs(direction.Y);
            double samplePDF = 0.5 / Math.PI;
            Ray scatteredRay = new(hit.Position, direction);
            var srcColor = RayColor(scatteredRay, depth - 1, 1) / samplePDF;
            reflectionEstimate += hit.Material.Reflect(r, hit, srcColor, scatteredRay);
        }
        reflectionEstimate /= samples;


        return color + reflectionEstimate;
    }
...
Parallel.For(0, height, y => 
{
    ...
    for (int x = 0; x < width; x++)
    {
        ...
        for (int sample = 0; sample < samples/4; sample++)
        {
            ...
            avgCol += RayColor(ray, maxDepth, 4);
        }
        avgCol = (avgCol / samples*4).Sqrt();
